  ...
  R: Clone,
{
  ...
  let post_fn_name = format!("__post{}", name);

  type PostFn<R> = unsafe extern "C" fn(*mut R);
  let post_fn = get_library_export::<PostFn<R>>(library, &post_fn_name);
  ...
  let return_value = if let Ok(post_fn) = post_fn {
    ...
    println!("[host] calling {name} with post");
  ...
  // else return value is simple Copy type
    ...
    // TEST
    println!("[host] calling {name} without post");
