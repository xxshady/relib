  ...
  for item in &exports_trait.items {
    ...
    let (decl, init, impl_) = if pub_exports {
      ...
      let (post_decl, post_init, return_type, read_return_value) = if needs_box {
        (
          quote! {
            #post_ident: unsafe extern "C" fn( *mut #pub_return_type ),
          },
          quote! {
            #post_ident: unsafe {
      ...
      (
        ...
        quote! {
          ...
          /// ```
          #[doc = #SAFETY_DOC]
          pub unsafe fn #ident<'module>( &'module self, #inputs ) -> Option<#pub_return_type> {
      ...
      (
        ...
        quote! {
          #[doc = #SAFETY_DOC]
          pub unsafe fn #ident( &self, #inputs ) -> #pub_return_type {
    ...
    let impl_code = if pub_imports {
      ...
      let (return_type, return_value, post_init) = if needs_box {
        (
          ...
          quote! {
            let post_ptr: *mut unsafe extern "C" fn(return_value_ptr: *mut #return_type)
              = *library.get(concat!(#post_mangled_name, "\0").as_bytes()).expect(#post_panic_message);

            *post_ptr = post_impl;

            unsafe extern "C" fn post_impl(return_value_ptr: *mut #return_type) {
              use std::boxed::Box;
